...
NSArray *PerformXPathQuery(xmlDocPtr doc, NSString *query, NSString *ns)
{
	...
	if(ns != nil){
		xmlXPathRegisterNs(xpathCtx, (xmlChar *)"n", (xmlChar *)[ns cStringUsingEncoding:NSUTF8StringEncoding]);
	}
...
NSArray *PerformHTMLXPathQuery(NSData *document, NSString *query, NSString *ns)
{
	...
	NSArray *result = PerformXPathQuery(doc, query, ns);
    xmlFreeDoc(doc); 
